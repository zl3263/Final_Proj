...
Manhattan_data = transformed_rental_income %>%
  ...
  mutate(
    address = paste(address,"Staten Island",sep = ",")
  )

Manhattan_data$latlona = map(.x = Manhattan_data$address, ~ geocode(.x, output = "latlona", source = "google"))
save(Manhattan_data, file = "data/Staten_Island_latlonmap.RData")

#

#load("E:/2023_autumn/DataScience/Final_Proj/data/Manhattan_latlonmap.RData")
#again?
load("data/Staten_Island_latlonmap.RData")


colnames(Manhattan_data) = c("address1","times","latlona")
Staten_Island_data = Manhattan_data %>%
  unnest(cols = latlona) %>%
  select(-address)
colnames(Staten_Island_data) = c("address","times","lon","lat")
save(Staten_Island_data, file = "data/Staten_Island_latlonmap.RData")
```
